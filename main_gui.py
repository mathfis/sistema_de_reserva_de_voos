...
class TelaPainel(tk.Frame):
    ...
    def ir_reservas(self):
        ...
        tk.Button(janela_reservas, text="Fechar", command=janela_reservas.destroy,
                font=("Arial", 10), bg="#e74c3c", fg="white", width=15).pack(pady=10)
    ...
    def carregar_minhas_reservas(self, parent, janela_pai):
        ...
        for i, reserva in enumerate(reservas_ativas):
            ...
            tk.Button(frame_reserva, text="Cancelar", width=8,
                    ...
                    bg="#e74c3c", fg="white", font=("Arial", 8)).grid(row=0, column=5, padx=2, pady=5)
    ...
    def cancelar_reserva(self, voo_id, assento_id, janela_pai):
        ...
        try:
            # 1. Cancelar no sistema do usuário
            usuario.cancelar_reserva(voo_id, assento_id)
            
            # 2. Remover do sistema do voo
            ...
            # 3. Atualizar status na lista de reservas do voo
            ...
            # ✅ CORREÇÃO: Salvar TODOS os usuários atualizados
...
class TelaAssentos(tk.Frame):
    ...
    def criar_widgets(self):
        ...
        # ✅ CORREÇÃO: FRAME SCROLLABLE PARA ASSENTOS
        ...
        # Canvas e Scrollbar
        canvas = tk.Canvas(container_frame, bg='#f0f0f0', height=300)  # Altura fixa para scroll
        scrollbar = ttk.Scrollbar(container_frame, orient="vertical", command=canvas.yview)
        
        # Frame scrollable dentro do canvas
        self.assentos_frame = tk.Frame(canvas, bg='#f0f0f0')
        ...
        tk.Button(btn_frame, text="Voltar ao Painel", command=self.voltar_painel,
                font=('Arial', 10), bg='#e74c3c', fg='white', padx=15).grid(row=0, column=0, padx=10)
        
        tk.Button(btn_frame, text="Ver Outros Voos", command=self.voltar_voos,
                font=('Arial', 10), bg='#f39c12', fg='white', padx=15).grid(row=0, column=1, padx=10)
        
        self.btn_confirmar = tk.Button(btn_frame, text="Confirmar Reserva", 
                                    command=self.confirmar_reserva, state=tk.DISABLED,
                                    font=('Arial', 10), bg='#27ae60', fg='white', padx=15)
    ...
    def criar_legenda(self, parent):
        legenda_frame = tk.Frame(parent, bg='#f0f0f0')
        ...
        for i, (texto, cor) in enumerate(legendas):
            frame = tk.Frame(legenda_frame, bg='#f0f0f0')
            frame.grid(row=0, column=i, padx=10)
            
            tk.Label(frame, text="■", fg=cor, font=('Arial', 14), bg='#f0f0f0').pack(side=tk.LEFT)
            tk.Label(frame, text=texto, font=('Arial', 9), bg='#f0f0f0').pack(side=tk.LEFT, padx=2)
    
    def carregar_assentos(self):
        if not self.master.voo_selecionado:
            return

        # CORREÇÃO: Atualizar informações do voo com atributo correto
        voo = self.master.voo_selecionado
        info_text = f"Voo: {voo.voo_id} | {voo.origem} → {voo.destino} | Avião: {voo.aviao.aviao_id}"
        # Atualizar o label existente (se necessário)
        ...
        for row_idx, (fileira, assentos) in enumerate(fileiras_ordenadas):
            ...
            tk.Label(self.assentos_frame, text=fileira, font=('Arial', 10, 'bold'), 
                    bg='#f0f0f0', width=3).grid(row=row_idx, column=0, padx=5, pady=2)
    ...
    def criar_botao_assento(self, assento_id, info, row, col):
        ...
        if info.get('bloqueado', False):
            cor = "#95a5a6"  # Cinza - bloqueado
            estado = tk.DISABLED
        elif self.assento_esta_reservado(assento_id):
            cor = "#e74c3c"  # Vermelho - ocupado
            estado = tk.DISABLED
        elif info.get('emergencia', False):
            cor = "#f39c12"  # Laranja - emergência
            estado = tk.NORMAL
        else:
            cor = "#2ecc71"  # Verde - livre
            estado = tk.NORMAL
    ...
    def assento_esta_reservado(self, assento_id):
        ...
        # CORREÇÃO: Verificar no sistema de reservas do voo
        voo = self.master.voo_selecionado
        
        # Verificar em assentos_reservados (novo sistema)
        if hasattr(voo, 'assentos_reservados') and assento_id in voo.assentos_reservados:
            return True
            
        # Verificar em reservas (sistema antigo)
        if hasattr(voo, 'reservas'):
            for reserva in voo.reservas:
    ...
    def selecionar_assento(self, assento_id, assento_info):
        ...
        self.assento_selecionado = assento_id  # CORREÇÃO: armazenar apenas o ID
    ...
    def voltar_painel(self):
        """Volta diretamente para o painel do usuário - MÉTODO CORRETO PARA TelaAssentos"""
        self.master.mostrar_tela("TelaPainel")

    def voltar_voos(self):
        self.master.mostrar_tela("TelaVoos")

    # ADICIONAR: Atualização de informações do voo na tela de assentos
    def atualizar_info_voo(self, voo):
        """Atualiza informações do voo na tela (método compatível)"""
        self.master.voo_selecionado = voo
        self.atualizar_tela()
    ...
    def confirmar_reserva(self):
        """Implementação CORRETA da reserva - VERSÃO SINCRONIZADA"""
        ...
        try:
            # ✅ CORREÇÃO: Fluxo unificado de reserva
            # 1. Reservar no sistema do voo
            sucesso = voo.reservar_assento(usuario, assento_id)
            
            if sucesso:
                # 2. Reservar no sistema do usuário
                layout = voo.aviao.gerar_layout()
                assento_info = layout.get(assento_id)
                usuario.criar_reserva(voo.voo_id, assento_id, assento_info)
                
                # ✅ CORREÇÃO CRÍTICA: Atualizar o voo no gerenciador
                self.master.gerenciador_voos.voos[voo.voo_id] = voo
                
                # 3. Salvar alterações em AMBOS os sistemas
                self.master.gerenciador_voos.salvar_voos()
                
                # ✅ CORREÇÃO: Salvar TODOS os usuários
                todos_usuarios = carregar_usuarios()
                ...
                # ✅ ADICIONAR: Atualizar painel do usuário
